    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <table className="w-full text-sm">
          <thead className="border-b border-slate-200 bg-slate-50"><tr>{sortHeading("account", "Account")}{sortHeading("expenses", "Expenses", true)}{sortHeading("purchases", "Purchases", true)}{sortHeading("total", "Total", true)}</tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={4} className="p-8 text-center text-slate-500">Loading postings…</td></tr> : !rows.length ? <tr><td colSpan={4} className="p-8 text-center text-slate-500">No posted expenses or purchases in this period.</td></tr> : sortedRows.map((row) => <tr key={row.id} className="border-b border-slate-100"><td className="p-3"><button type="button" onClick={() => toggleSort("code")} className="font-mono text-xs text-slate-500" title="Sort by account code">{row.code || "—"}</button> <span className="font-medium text-slate-800">{row.account}</span></td><td className="p-3 text-right">{money(row.expenses)}</td><td className="p-3 text-right">{money(row.purchases)}</td><td className="p-3 text-right font-semibold">{money(row.total)}</td></tr>)}
          </tbody>
