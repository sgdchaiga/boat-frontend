...
export function VslaShareOutPage({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    setFundTotal(
      Number(
        (fRes.data?.[0] as { fund_total?: number } | undefined)?.fund_total ??
          0,
      ),
    );
    setError(
      mRes.error?.message ?? sRes.error?.message ?? fRes.error?.message ?? null,
    );
  }, [orgId, superAdmin]);
  ...
  const totalShares = useMemo(
    () => shares.reduce((s, x) => s + x.shares, 0),
    [shares],
  );
  ...
  const runShareOut = async () => {
    ...
    if (fundTotal <= 0 || totalShares <= 0) {
      setError(
        "Provide fund total and ensure there are shares before share-out.",
      );
      return;
    }
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 grid md:grid-cols-4 gap-3">
        <label className="text-xs text-slate-600 md:col-span-2">
          ...
          <input
            type="number"
            value={fundTotal}
            onChange={(e) => setFundTotal(Number(e.target.value || 0))}
            className="mt-1 w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
          />
        </label>
        <div className="text-sm p-3 rounded-lg bg-slate-100">
          Total Shares: <strong>{totalShares}</strong>
        </div>
        <div className="text-sm p-3 rounded-lg bg-slate-100">
          Value/Share: <strong>{valuePerShare.toFixed(2)}</strong>
        </div>
        <div className="md:col-span-4">
          <button
            type="button"
            onClick={() => void runShareOut()}
            disabled={readOnly}
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="text-left p-3">Projected Payout</th>
            </tr>
          </thead>
          <tbody>
            {shares.map((s) => (
              <tr key={s.member_id} className="border-b border-slate-100">
                <td className="p-3">
                  {memberName.get(s.member_id) ?? "Unknown"}
                </td>
                <td className="p-3">{s.shares}</td>
                <td className="p-3">
                  {(s.shares * valuePerShare).toLocaleString()}
                </td>
              </tr>
            ))}
          </tbody>
