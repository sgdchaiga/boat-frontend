...
type LoanRow = { id: string; principal_amount: number; outstanding_balance: number; status: string; due_date: string | null; applied_at: string };
...
export default function VslaMemberDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    const [shareResult, loanResult, fineResult, meetingResult] = await Promise.all([
      ...
      supabase.from("vsla_loans").select("id,principal_amount,outstanding_balance,status,due_date,applied_at").eq("member_id", memberId).order("applied_at", { ascending: false }),
