...
type WelfareStampRow = { meeting_id: string | null; total_value: number };
...
export function VslaCashboxPage({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const load = useCallback(async () => {
    const [
      ...
      filterByOrganizationId(
        supabase
          ...
          .select("meeting_id,principal_paid,interest_paid,penalty_paid"),
        orgId,
        superAdmin,
      ),
      filterByOrganizationId(
      ...
      filterByOrganizationId(
        supabase
          ...
          .select("meeting_id,fund_type,txn_type,amount"),
        orgId,
        superAdmin,
      ),
      filterByOrganizationId(
        supabase
          .from("vsla_loans")
          .select("principal_amount,status,disbursed_on"),
        orgId,
        superAdmin,
      ),
      filterByOrganizationId(
    ...
    setWelfareStamps((welfareRes.data ?? []) as WelfareStampRow[]);
    setError(
  ...
  const computeByMeeting = useCallback(
    (targetMeetingId: string | null | undefined) => {
      ...
      const outflowLoans = meetingTxns
        .filter(
          (r) => r.meeting_id === targetMeetingId && r.kind === "loan_issue",
        )
        .reduce((s, r) => s + Number(r.amount || 0), 0);
      ...
      const meetingDate =
        meetings.find((m) => m.id === targetMeetingId)?.meeting_date ?? null;
        ...
        meetingDate == null
          ...
          : loans
              .filter(
                (l) =>
                  l.status === "disbursed" &&
                  String(l.disbursed_on || "").slice(0, 10) === meetingDate,
              )
              .reduce((s, l) => s + Number(l.principal_amount || 0), 0);
      const outflowSocialFromMeetingTxns = meetingTxns
        .filter(
          (r) => r.meeting_id === targetMeetingId && r.kind === "social_payout",
        )
        .reduce((s, r) => s + Number(r.amount || 0), 0);
      const outflowSocialFromFundTxns = fundTxns
        .filter(
          (r) =>
            ...
            r.fund_type === "social_fund" &&
            r.txn_type === "payout",
        )
        .reduce((s, r) => s + Number(r.amount || 0), 0);
      const inflowChairmanBasket = meetingTxns
        .filter(
          (r) =>
            r.meeting_id === targetMeetingId && r.kind === "chairman_basket",
        )
        .reduce((s, r) => s + Number(r.amount || 0), 0);
      const outflowRefreshments = meetingTxns
        .filter(
          (r) => r.meeting_id === targetMeetingId && r.kind === "refreshments",
        )
        .reduce((s, r) => s + Number(r.amount || 0), 0);
      const inflowSocialWelfare = welfareStamps
        .filter((r) => r.meeting_id === targetMeetingId)
        .reduce((s, r) => s + Number(r.total_value || 0), 0);

      return {
        inflowSavings: inflowSavings + inflowChairmanBasket,
        inflowSocialWelfare,
        inflowRepayments,
        inflowFines: inflowFinesFromFinesTable + inflowFinesFromMeetingTxns,
        outflowLoans: outflowLoans + outflowLoansFromDisbursedLoans,
        outflowSocialPayouts:
          outflowSocialFromMeetingTxns +
          outflowSocialFromFundTxns +
          outflowRefreshments,
      };
    },
    [
  ...
  const latestCalc = useMemo(
    () => computeByMeeting(latestSnapshot?.meeting_id),
    [computeByMeeting, latestSnapshot?.meeting_id],
  );
  const expected = useMemo(() => {
    ...
    return (
      ...
      latestCalc.inflowSocialWelfare +
  ...
  const latestVariance = latestSnapshot
    ? Number(latestSnapshot.physical_cash || 0) - expected
    : 0;

  return (
    <div className="px-4 py-6 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
      {readOnly && <ReadOnlyNotice />}
      <div>
        <h1 className="text-2xl font-bold text-slate-900">
          VSLA Cashbox / Fund Tracking
        </h1>
        <p className="text-sm text-slate-600 mt-1">
          Track opening cash, compare expected vs physical cash, and monitor
          meeting cash variance.
        </p>
      </div>
        ...
        <div className="grid md:grid-cols-4 gap-3">
          ...
          <div className="flex items-end">
            <button
              ...
              disabled={readOnly}
              className="w-full px-4 py-2 bg-indigo-700 text-white rounded-lg text-sm disabled:opacity-50"
            >
              Save Snapshot
            </button>
          </div>
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <div className="p-3 rounded-lg bg-slate-100 text-sm">
          Expected Cash (system): <strong>{expected.toLocaleString()}</strong>
        </div>
        <div className="p-3 rounded-lg bg-slate-100 text-sm">
          Physical Cash (counted):{" "}
          <strong>
            {Number(latestSnapshot?.physical_cash || 0).toLocaleString()}
          </strong>
        </div>
        <div
          className={`p-3 rounded-lg text-sm ${Math.abs(latestVariance) > 0 ? "bg-rose-100 text-rose-800" : "bg-emerald-100 text-emerald-800"}`}
        >
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        ...
        <div className="p-4 grid md:grid-cols-3 gap-3 text-sm">
          <div className="rounded-lg bg-slate-50 p-3">
            Opening Cash:{" "}
            <strong>
              {Number(latestSnapshot?.opening_cash || 0).toLocaleString()}
            </strong>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            Savings Inflow (calculated):{" "}
            <strong>{latestCalc.inflowSavings.toLocaleString()}</strong>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            Social welfare stamps (calculated):{" "}
            <strong>{latestCalc.inflowSocialWelfare.toLocaleString()}</strong>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            Repayments Inflow (calculated):{" "}
            <strong>{latestCalc.inflowRepayments.toLocaleString()}</strong>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            Fines Inflow (calculated):{" "}
            <strong>{latestCalc.inflowFines.toLocaleString()}</strong>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            Loans Outflow (calculated):{" "}
            <strong>{latestCalc.outflowLoans.toLocaleString()}</strong>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            Social Payouts Outflow (calculated):{" "}
            <strong>{latestCalc.outflowSocialPayouts.toLocaleString()}</strong>
          </div>
        </div>
      </div>
    </div>
