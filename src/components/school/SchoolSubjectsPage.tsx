...
export function SchoolSubjectsPage({ readOnly }: Props) {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    if (canUseSchoolApi()) {
      try {
        const data = await listSchoolRows<Row>("subjects", orgId);
        setRows(data);
        setErr(null);
      } catch (error) {
        setErr(error instanceof Error ? error.message : "Failed to load subjects.");
      } finally {
        setLoading(false);
      }
      return;
  ...
  const save = async () => {
    ...
    if (canUseSchoolApi()) {
      try {
        await createSchoolRow<Row>("subjects", user?.organization_id || "", payload);
        setForm({ name: "", code: "", sort_order: "0" });
        load();
      } catch (error) {
        setErr(error instanceof Error ? error.message : "Failed to save subject.");
      }
      return;
    }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <PageNotes ariaLabel="Subjects">
          <p>Curriculum subjects (e.g. Mathematics, English). Use for timetabling and reporting when you extend the module.</p>
        </PageNotes>
      </div>
