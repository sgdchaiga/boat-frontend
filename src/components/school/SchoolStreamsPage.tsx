...
export function SchoolStreamsPage({ readOnly }: Props) {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    if (canUseSchoolApi()) {
      try {
        const data = await listSchoolRows<Row>("streams", orgId);
        setRows(data);
        setErr(null);
      } catch (error) {
        setErr(error instanceof Error ? error.message : "Failed to load streams.");
      } finally {
        setLoading(false);
      }
      return;
  ...
  const save = async () => {
    ...
    if (canUseSchoolApi()) {
      try {
        await createSchoolRow<Row>("streams", user?.organization_id || "", payload);
        setForm({ name: "", code: "", sort_order: "0" });
        load();
      } catch (error) {
        setErr(error instanceof Error ? error.message : "Failed to save stream.");
      }
      return;
    }
  ...
  const saveEdit = async () => {
    ...
    if (canUseSchoolApi()) {
      try {
        await updateSchoolRow<Row>("streams", user?.organization_id || "", editingId, payload);
        cancelEdit();
        load();
      } catch (error) {
        setErr(error instanceof Error ? error.message : "Failed to update stream.");
      }
      return;
    }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <PageNotes ariaLabel="Streams">
          <p>Tracks or streams (e.g. Science, Arts, Red). Assign students to a stream alongside their class.</p>
        </PageNotes>
      </div>
