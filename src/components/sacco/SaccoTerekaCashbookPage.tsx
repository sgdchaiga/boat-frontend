...
import { ArrowDown, ArrowUp, ArrowUpDown, BookOpen, Palette, Plus, RefreshCw, SlidersHorizontal, X } from "lucide-react";
...
export function SaccoTerekaCashbookPage({ view, navigate }: { view: View; navigate: (page: string, state?: Record<string, unknown>) => void }) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5 p-4 sm:p-6 lg:p-8">
    ...
    {view === "register" && <CashbookRegister rows={rows} members={members} loans={loans} loading={loading} preferenceKey={`boat.sacco.cashbook.columns.${orgId || "no-org"}.${user?.id || "anonymous"}`} headerColor={appearance.primary_color} />}

    {view === "daily" && <section className="rounded-2xl border bg-white p-5 shadow-sm"><label className="text-sm font-semibold">Summary date<input type="date" value={summaryDate} onChange={(e) => setSummaryDate(e.target.value)} className="ml-3 rounded-lg border px-3 py-2" /></label><div className="mt-4 overflow-x-auto rounded-xl border"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-slate-900 text-xs uppercase text-white"><tr><th className="px-3 py-3 text-left">Details</th><th>Date</th><th className="text-right">Airtel Money</th><th className="text-right">MTN MoMo</th><th className="text-right">Bank</th><th className="text-right">Cash In</th><th className="text-right">Cash Out</th><th className="px-3 text-right">Balance</th></tr></thead><tbody><tr className="bg-slate-50 font-semibold"><td className="px-3 py-2">Balance b/f</td><td>{summaryDate}</td><td/><td/><td/><td/><td/><td className="px-3 text-right">{money.format(opening)}</td></tr>{dailyRows.map(r => { running += r.net_amount; return <tr key={r.id} className="border-t"><td className="px-3 py-2"><b>{r.narration}</b><span className="block text-xs text-slate-500">{r.client_name}</span></td><td>{r.transaction_date}</td><ChannelCell show={r.payment_channel === "airtel_money"} n={r.net_amount}/><ChannelCell show={r.payment_channel === "mtn_momo"} n={r.net_amount}/><ChannelCell show={r.payment_channel === "bank"} n={r.net_amount}/><MoneyCell n={r.payment_channel === "cash" ? r.deposit_amount : 0}/><MoneyCell n={r.payment_channel === "cash" ? r.withdraw_amount : 0}/><td className="px-3 py-2 text-right font-bold">{money.format(running)}</td></tr>})}</tbody></table></div><p className="mt-3 text-xs text-slate-500">Existing teller-synced lines remain available in the system journal ({cashbook.length} lines); direct cashbook entries are reported here by payment channel.</p></section>}
  </div>;
}
...
function CashbookRegister({ rows, members, loans, loading, preferenceKey, headerColor }: { rows: Row[]; members: Array<{ id: string; name: string; accountNumber?: string }>; loans: Array<{ id: string; loanNumber?: string; memberName: string }>; loading: boolean; preferenceKey: string; headerColor: string }) {
  ...
  return <section className="rounded-2xl border bg-white shadow-sm">
    ...
    <div className="flex flex-wrap justify-between gap-2 border-b bg-slate-50 px-4 py-2 text-xs text-slate-600"><span>{filtered.length} of {rows.length} entries</span><span>Click a heading to sort. Column choices are saved for your next login.</span></div>
    <div className="max-h-[68vh] overflow-auto"><table className="w-full min-w-max text-sm"><thead className="sticky top-0 z-10 text-xs uppercase text-white" style={{ backgroundColor: headerColor }}><tr>{visible.map((id, index) => <th key={id} className={`whitespace-nowrap px-3 py-3 text-left ${index === 0 ? "sticky left-0 z-20" : ""}`} style={index === 0 ? { backgroundColor: headerColor } : undefined}><button type="button" onClick={() => toggleSort(id)} className="inline-flex items-center gap-1 font-semibold">{COLUMN_LABELS[id]}{sort.id === id ? sort.direction === "asc" ? <ArrowUp className="h-3.5 w-3.5" /> : <ArrowDown className="h-3.5 w-3.5" /> : <ArrowUpDown className="h-3.5 w-3.5 opacity-60" />}</button></th>)}</tr></thead><tbody className="divide-y">{filtered.map(row => <tr key={row.id} className="hover:bg-slate-50">{visible.map((id, index) => <td key={id} className={`whitespace-nowrap px-3 py-2 ${["deposit", "withdraw", "net"].includes(id) ? "text-right tabular-nums" : ""} ${index === 0 ? "sticky left-0 bg-white" : ""}`}>{display(row, id)}</td>)}</tr>)}{!loading && filtered.length === 0 && <tr><td colSpan={visible.length} className="px-4 py-10 text-center text-slate-500">No cashbook entries match these filters.</td></tr>}</tbody></table></div>
  </section>;
}
