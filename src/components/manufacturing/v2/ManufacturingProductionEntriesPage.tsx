...
type WorkOrderRow = { id: string; product_name: string; bom_id?: string | null; status?: string | null };
...
type LotRow = { id: string; product_id: string; lot_number: string; status: string };
type PendingProductionItem = {
  ...
  scrapQty: string;
  outputLotId: string;
  materialLots: Record<string, string>;
};
...
export function ManufacturingProductionEntriesPage({ readOnly = false, simpleMode = false }: { readOnly?: boolean; simpleMode?: boolean }) {
  ...
  const [materialLots, setMaterialLots] = useState<Record<string, string>>({});
  ...
  const loadRefs = async () => {
    ...
    const [woRes, staffRes, productRes, bomRes, lotsRes] = await Promise.all([
      filterByOrganizationId(supabase.from("manufacturing_work_orders").select("id,product_name,bom_id,status").order("created_at", { ascending: false }), orgId, superAdmin),
      ...
      filterByOrganizationId(supabase.from("product_lots").select("id,product_id,lot_number,status").in("status", ["available", "consumed"]).order("lot_number"), orgId, superAdmin),
    ]);
  ...
  const startEdit = (row: Record<string, unknown>) => {
    ...
    setOutputLotId(String(row.output_lot_id || ""));
    setMaterialLots((row.material_lots && typeof row.material_lots === "object" ? row.material_lots : {}) as Record<string, string>);
  ...
  const validateProductionItem = (item: PendingProductionItem): string | null => {
    ...
    const order = workOrders.find((row) => row.id === item.workOrderId);
    const itemBom = boms.find((bom) => bom.id === order?.bom_id)
      || boms.find((bom) => bom.product_id === item.productId && bom.status === "Active")
  ...
  const handleSave = async () => {
    ...
    const payloads = draftItems.map((item) => {
      ...
      const payload: Record<string, unknown> = {
        ...
        output_lot_id: item.outputLotId || null,
        material_lots: item.materialLots,
  ...
  const selectedWorkOrder = workOrders.find((order) => order.id === workOrderId);
  const selectedWorkOrderBom = boms.find((bom) => bom.id === selectedWorkOrder?.bom_id);
  const materialBom = selectedWorkOrderBom || selectedBom;
  const materialRows = (materialBom?.materials || []) as Array<{ item_id: string; item_name: string; qty: number; unit: string }>;
    ...
    <div className="p-6 md:p-8">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="text-xs text-slate-600">{simpleMode ? "Output product" : "Finished product"}<select value={productId} disabled={!simpleMode && !!workOrderId} onChange={(e) => setProductId(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-100"><option value="">Select output product...</option>{(simpleMode ? allProducts : products).map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select>{!simpleMode && <span className="mt-1 block text-[11px] text-slate-400">{productId ? selectedWorkOrderBom ? `Uses production-order BOM ${selectedWorkOrderBom.version}: ${selectedWorkOrderBom.materials.length} material(s) per ${selectedWorkOrderBom.output_qty} output.` : selectedBom ? `Uses ${selectedBom.status} BOM ${selectedBom.version}: ${selectedBom.materials.length} material(s) per ${selectedBom.output_qty} output.` : "No BOM found. Create a BOM before recording production." : "Select a product to see its BOM connection."}</span>}</label>
          {!simpleMode && <label className="text-xs text-slate-600">Finished-goods lot<select value={outputLotId} onChange={(e) => setOutputLotId(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="">No lot selected</option>{lots.filter((lot) => lot.product_id === productId && lot.status === "available").map((lot) => <option key={lot.id} value={lot.id}>{lot.lot_number}</option>)}</select><span className="mt-1 block text-[11px] text-slate-400">Create lots in Manufacturing → Lots & quality before recording batch output.</span></label>}
          {!simpleMode && materialRows.length > 0 && <div className="md:col-span-2 lg:col-span-3 rounded-lg border border-slate-200 bg-slate-50 p-3"><p className="mb-2 text-xs font-semibold text-slate-700">Raw-material lots for this batch</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{materialRows.map((material) => <label key={material.item_id} className="text-xs text-slate-600">{material.item_name || "Material"}<select value={materialLots[material.item_id] || ""} onChange={(e) => setMaterialLots((current) => ({ ...current, [material.item_id]: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="">Untracked stock</option>{lots.filter((lot) => lot.product_id === material.item_id && lot.status === "available").map((lot) => <option key={lot.id} value={lot.id}>{lot.lot_number}</option>)}</select></label>)}</div><p className="mt-2 text-[11px] text-slate-500">Selected lots must have enough lot-tagged stock for the BOM quantity. Untracked stock continues to use the existing inventory flow.</p></div>}
          ...
          {!simpleMode && <label className="text-xs text-slate-600">Production order (optional)<select value={workOrderId} onChange={(e) => { const nextOrder = workOrders.find((order) => order.id === e.target.value); setWorkOrderId(e.target.value); const bom = boms.find((row) => row.id === nextOrder?.bom_id); if (bom) setProductId(bom.product_id); }} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="">No production order</option>{workOrders.filter((order) => order.status !== "Cancelled" && order.status !== "Completed").map((order) => <option key={order.id} value={order.id}>{order.product_name}{order.status ? ` · ${order.status}` : ""}</option>)}</select>{selectedWorkOrder ? <span className="mt-1 block text-[11px] text-slate-500">Uses order BOM {selectedWorkOrderBom?.version || "linked recipe"}; product selection is locked.</span> : null}</label>}
