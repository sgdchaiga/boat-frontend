...
import { Cloud, CloudOff, Download, Gauge, Lock, RefreshCw, Settings2, X } from "lucide-react";
...
export function MobileLiteCenter(props: Props) {
  ...
  const pending = readOfflineRetailQueue().length + QUEUE_KEYS.reduce((sum, key) => sum + storedArrayCount(key), 0);
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          <section className={`rounded-xl border p-3 ${online ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1"><p className="text-sm font-bold">{online ? "Online" : "Working offline"}</p><p className="text-xs text-slate-600">{pending} item{pending === 1 ? "" : "s"} waiting on this device</p></div>
          ...
          <section className="grid grid-cols-2 gap-2">
            ...
            <button type="button" onClick={props.onLock} className="min-h-16 rounded-xl border bg-white p-3 text-left text-sm font-semibold"><Lock className="mb-1 h-5 w-5 text-slate-700" />Lock now</button>
          </section>
