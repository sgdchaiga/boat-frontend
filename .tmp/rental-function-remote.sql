...
BEGIN
  ...
  VALUES (v_org, 'RENT-' || to_char(p_month, 'YYYYMM') || '-' || p_property_id::text,
    ...
    p_property_id, p_month, 'Monthly rent: ' || v_property.name || ' Â· ' || to_char(p_month, 'FMMonth YYYY'))
  RETURNING * INTO v_invoice;
  INSERT INTO public.retail_invoice_lines (invoice_id, line_no, description, quantity, unit_price, line_total)
    VALUES (v_invoice.id, 1, 'Rent: ' || v_property.name || ' Â· ' || to_char(p_month, 'FMMonth YYYY'), 1, v_amount, v_amount);
