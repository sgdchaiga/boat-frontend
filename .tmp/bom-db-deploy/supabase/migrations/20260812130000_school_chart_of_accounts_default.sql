...
-- Mark legacy industry templates so school screens can consistently exclude them.
-- Accounts are retained for audit history and can still be reclassified by an administrator.
UPDATE public.gl_accounts AS account
SET business_type = 'manufacturing'
FROM public.organizations AS organization
WHERE account.organization_id = organization.id
  AND lower(COALESCE(organization.business_type, '')) = 'school'
  AND lower(COALESCE(account.business_type, '')) <> 'school'
  AND concat_ws(' ', account.account_name, account.category) ~* '(raw materials?|work in progress|finished goods|factory overhead|production overhead|production clearing|cost of goods manufactured|scrap inventory)';
