-- Automate BOM consumption and material costing when production output is recorded.
...
ALTER TABLE public.manufacturing_production_entries
  ...
  ADD COLUMN IF NOT EXISTS material_cost numeric(18,2) NOT NULL DEFAULT 0 CHECK (material_cost >= 0);

ALTER TABLE public.manufacturing_costing_entries
  ADD COLUMN IF NOT EXISTS production_entry_id uuid REFERENCES public.manufacturing_production_entries(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS bom_id uuid REFERENCES public.manufacturing_boms(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS product_id uuid REFERENCES public.products(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS generated_from_production boolean NOT NULL DEFAULT false;

CREATE UNIQUE INDEX IF NOT EXISTS idx_manufacturing_costing_production_entry
  ON public.manufacturing_costing_entries (production_entry_id)
  WHERE production_entry_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_manufacturing_production_bom
  ON public.manufacturing_production_entries (organization_id, bom_id, production_date DESC);
...
BEGIN
  ...
  LOOP
    ...
    IF v_material_id IS NULL OR v_qty <= 0 THEN
      CONTINUE;
    END IF;

    -- Serialize production postings that consume the same material.
    PERFORM 1
  ...
  INSERT INTO public.manufacturing_costing_entries (
    ...
    period, product_name, material_cost, labor_cost, overhead_cost,
    generated_from_production
  ) VALUES (
    NEW.organization_id,
    NEW.id,
    v_bom.id,
  ...
  IF NEW.work_order_id IS NOT NULL THEN
    ...
    SET completed_qty = completed_qty + NEW.produced_qty,
        status = CASE
          WHEN completed_qty + NEW.produced_qty >= planned_qty THEN 'Completed'
          ELSE 'In Progress'
        END
...
AFTER INSERT ON public.manufacturing_production_entries
FOR EACH ROW
EXECUTE FUNCTION public.post_manufacturing_bom_consumption();

COMMENT ON COLUMN public.manufacturing_production_entries.bom_id IS 'BOM automatically selected when the production entry was posted.';
COMMENT ON COLUMN public.manufacturing_production_entries.material_cost IS 'Material cost calculated from scaled BOM quantities and item cost prices.';
