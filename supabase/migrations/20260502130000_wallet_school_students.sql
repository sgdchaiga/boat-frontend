-- School: wallets belong to students (same liability/clearing GL pattern as other customers).

ALTER TABLE public.wallets
  ADD COLUMN IF NOT EXISTS student_id uuid REFERENCES public.students(id) ON DELETE CASCADE;

ALTER TABLE public.wallets DROP CONSTRAINT IF EXISTS wallets_customer_check;
ALTER TABLE public.wallets DROP CONSTRAINT IF EXISTS wallets_customer_kind_check;

ALTER TABLE public.wallets
  ADD CONSTRAINT wallets_customer_kind_check CHECK (customer_kind IN ('hotel', 'retail', 'student'));

ALTER TABLE public.wallets
  ADD CONSTRAINT wallets_customer_check CHECK (
    (customer_kind = 'hotel' AND hotel_customer_id IS NOT NULL AND retail_customer_id IS NULL AND student_id IS NULL)
    OR (customer_kind = 'retail' AND retail_customer_id IS NOT NULL AND hotel_customer_id IS NULL AND student_id IS NULL)
    OR (customer_kind = 'student' AND student_id IS NOT NULL AND hotel_customer_id IS NULL AND retail_customer_id IS NULL)
  );

CREATE UNIQUE INDEX IF NOT EXISTS wallets_org_student_unique
  ON public.wallets (organization_id, student_id)
  WHERE student_id IS NOT NULL;

COMMENT ON COLUMN public.wallets.student_id IS 'School: wallet holder (student). Mutually exclusive with hotel/retail customer ids.';

-- GL dimensions: include student_id on journal lines
...
BEGIN
  ...
  v_dim_from := jsonb_strip_nulls(jsonb_build_object(
    ...
    'student_id', v_from_w.student_id
  ));
