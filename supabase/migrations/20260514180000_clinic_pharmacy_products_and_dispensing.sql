-- Clinic pharmacy: product expiry & drug category; link retail sales to clinic patients for dispensing history.

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS expiry_date date,
  ADD COLUMN IF NOT EXISTS drug_category text;

COMMENT ON COLUMN public.products.expiry_date IS 'Optional expiry / best-before for pharmacy stock (clinic tenants).';
COMMENT ON COLUMN public.products.drug_category IS 'Drug shelf category (e.g. antibiotics, injections).';
...
ALTER TABLE public.retail_sales
  ...
  ADD COLUMN IF NOT EXISTS clinic_diagnosis_snapshot text;

COMMENT ON COLUMN public.retail_sales.clinic_patient_id IS 'When set, this POS sale is a clinic dispensing for this patient.';
COMMENT ON COLUMN public.retail_sales.clinic_diagnosis_snapshot IS 'Diagnosis text copied at checkout for pharmacy receipt / audit.';

CREATE INDEX IF NOT EXISTS idx_retail_sales_clinic_patient_sale_at
  ON public.retail_sales(organization_id, clinic_patient_id, sale_at DESC)
  WHERE clinic_patient_id IS NOT NULL;

-- Replace single overload (parameter list changed).
...
-- Atomic POS post with optional clinic dispensing metadata on the sale header.
...
COMMENT ON FUNCTION public.post_retail_sale_atomic(
  uuid, uuid, uuid, uuid, text, text, numeric, numeric, numeric, numeric, text, boolean, numeric, uuid, jsonb, jsonb, date, text, jsonb, uuid, text
) IS 'Atomic retail sale post: retail_sales + lines + split payments + stock movements + journal; optional clinic patient/diagnosis on header.';
