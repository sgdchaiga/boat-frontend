-- Some deployments hit: ERROR 42P01 relation "v_smart" / "v_allowed" / etc. does not exist
-- when `SELECT ... INTO <var>` was parsed outside a proper plpgsql DECLARE block.
-- Re-define both RPCs: scalar `:=` / `EXISTS(...)`, stay row via FOR-loop + `v_stay_found` (no INTO st).
